...
function M.fetch(lat, lon, temp_unit, forecast_days, wind_unit, precip_unit)
    ...
    if current then
        local wmo = api.getWeatherInfo(current.weather_code)
        result.current = {
            ...
            wind_label = api.windDirectionLabel(current.wind_direction_10m),
            pressure = current.surface_pressure,
            is_day = current.is_day,
        }
        if not current.is_day or current.is_day == 0 then
            result.current.weather_icon = api.getNightIcon(result.current.weather_icon)
        end
        ...
        for i, t in ipairs(hourly.time) do
            ...
            if #parts >= 2 then
                ...
                if ts >= current_hour_ts then
                    local wmo = api.getWeatherInfo(hourly.weather_code[i])
                    local is_night = parts[4] and (parts[4] < 6 or parts[4] >= 18)
                    local icon = is_night and api.getNightIcon(wmo.icon) or wmo.icon
    ...
    if daily and daily.time then
        ...
        for i, t in ipairs(daily.time) do
            local wmo = api.getWeatherInfo(daily.weather_code[i])
            local parts = {}
            for part in t:gmatch("[%d]+") do
                table.insert(parts, tonumber(part))
            end
